...
void SerializeCatalogue(const transport::Catalogue& catalogue,
    const renderer::MapRenderer& map_renderer, const transport::Router& router,
    std::ostream& output) {
    serialize::TransportCatalogue database;
    for (const auto& [stop_name, stop] : catalogue.GetSortedAllStops()) {
        *database.add_stop() = SerializeStop(stop);
    }
    for (const auto& [bus_name, bus] : catalogue.GetSortedAllBuses()) {
        *database.add_bus() = SerializeBus(bus);
    }
    *database.mutable_render_settings() = GetSerializeRenderSettings(map_renderer.GetRenderSettings());
...
serialize::Stop SerializeStop(const transport::Stop* stop) {
    ...
    for (const auto& [name, distance] : stop->stop_distances) {
        result.add_near_stop(static_cast<string>(name));
        result.add_distance(distance);
    }
...
serialize::Point SerializePoint(const json::Array& point) {
    serialize::Point result;
    result.set_x(point[0].AsDouble());
    result.set_y(point[1].AsDouble());
    return result;
}
...
serialize::RenderSettings GetRenderSettingSerialize(const json::Node& render_settings) {
    const json::Dict& render_settings_dict = render_settings.AsDict();
    serialize::RenderSettings result;
    result.set_width(render_settings_dict.at("width"s).AsDouble());
    result.set_height(render_settings_dict.at("height"s).AsDouble());
    result.set_padding(render_settings_dict.at("padding"s).AsDouble());
    result.set_stop_radius(render_settings_dict.at("stop_radius"s).AsDouble());
    result.set_line_width(render_settings_dict.at("line_width"s).AsDouble());
    result.set_bus_label_font_size(render_settings_dict.at("bus_label_font_size"s).AsInt());
    *result.mutable_bus_label_offset() = GetPointSerialize(render_settings_dict.at("bus_label_offset"s).AsArray());
    result.set_stop_label_font_size(render_settings_dict.at("stop_label_font_size"s).AsInt());
    *result.mutable_stop_label_offset() = GetPointSerialize(render_settings_dict.at("stop_label_offset"s).AsArray());
    *result.mutable_underlayer_color() = GetColorSerialize(render_settings_dict.at("underlayer_color"s));
    result.set_underlayer_width(render_settings_dict.at("underlayer_width"s).AsDouble());
    for (const auto& color : render_settings_dict.at("color_palette"s).AsArray()) {
        *result.add_color_palette() = GetColorSerialize(color);
    }
...
void SetStopsDistances(transport::Catalogue& catalogue, const serialize::TransportCatalogue& database) {
...
void AddStopFromDB(transport::Catalogue& catalogue, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.stop_size(); ++i) {
        const serialize::Stop& stop_i = database.stop(i);
...
void AddBusFromDB(transport::Catalogue& catalogue, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.bus_size(); ++i) {
        ...
        catalogue.AddBus(bus_i.name(), stops, bus_i.is_circle());

        if (!bus_i.final_stop().empty()) {
            transport::Bus* bus = catalogue.FindBus(bus_i.name());
...
graph::DirectedWeightedGraph<double> GetGraphFromDB(const serialize::Router& router) {
    ...
    for (size_t i = 0; i < edges.size(); ++i) {
        ...
        edges[i] = {
            e.name(),
            static_cast<size_t>(e.quality()),
            static_cast<size_t>(e.from()),
