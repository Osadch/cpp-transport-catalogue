...
namespace transport {
    ...
    const graph::DirectedWeightedGraph<double>& Router::BuildGraph(const TransportCatalogue& catalogue) {
        // Получаем отсортированный список всех остановок и автобусов
        const auto& all_stops = catalogue.GetSortedAllStops();
        const auto& all_buses = catalogue.GetSortedAllBuses();
        // Создаем граф остановок и дорог с учетом размера списка остановок
        graph::DirectedWeightedGraph<double> stops_graph(all_stops.size() * 2);
        // Создаем map идентификаторов для остановок
        std::map<std::string, graph::VertexId> stop_ids;
        // Инициализируем идентификатор вершины
        graph::VertexId vertex_id = 0;
        // Добавляем ребра для каждой остановки на временную остановку
        // и обратное ребро для каждой временной остановки на прямую остановку
        ...
        // Добавляем ребра между остановками для каждого автобуса
        // Расстояния между остановками и время в пути вычисляются с помощью методов класса TransportCatalogue
        std::for_each(
            all_buses.begin(),
            all_buses.end(),
            [&stops_graph, this, &catalogue](const auto& item) {
                ...
                for (size_t i = 0; i < stops_count; ++i) {
                    for (size_t j = i + 1; j < stops_count; ++j) {
                        ...
                        // Вычисляем суммарное расстояние между остановками и обратное расстояние
                        for (size_t k = i + 1; k <= j; ++k) {
                            dist_sum += catalogue.GetDistance(stops[k - 1], stops[k]);
                            dist_sum_inverse += catalogue.GetDistance(stops[k], stops[k - 1]);
                        }
                        // Добавляем ребро между остановками и устанавливаем время в пути
                        ...
                        // Если автобус не кольцевой, добавляем обратное ребро
